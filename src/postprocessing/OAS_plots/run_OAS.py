...
def get_OAS_surface(num_y=15, num_x=5, symmetry=True):
    """
...
def setup_OAS_singlepoint_problem(num_y=15, num_x=5):
    # setup OAS single point model
    surface = get_OAS_surface(num_y=num_y, num_x=num_x, symmetry=False)
    ...
    indep_var_comp.add_output("re", val=4e5, units="1/m")
    indep_var_comp.add_output("rho", val=1.225, units="kg/m**3")
    indep_var_comp.add_output("CT", val=grav_constant * 17.0e-6, units="1/s")
    indep_var_comp.add_output("R", val=1000, units="m")
    indep_var_comp.add_output("W0", val=1.8 * 4, units="kg")
...
if __name__ == '__main__':
    ...
    prob, surface = setup_OAS_singlepoint_problem()
